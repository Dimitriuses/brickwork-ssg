...
The generators [generators/generate-products.build.js](generators/generate-products.build.js) (scans `build/products`) and [generators/generate-custom.build.js](generators/generate-custom.build.js) (scans `build/custom`) fill the engine templates [generators/_product-detail-template.html](generators/_product-detail-template.html) / [generators/_custom-detail-template.html](generators/_custom-detail-template.html), slugify the folder name into a selector-/URL-safe id (via [lib/slugify.js](lib/slugify.js)), and write one `_generated-product-<id>.json` into the temp `build/_generated-pages/`, which step 7 builds into `product-<id>.html`. Pages whose name starts with `product-` pick up `generators/product-detail.css|js`, resolved **site-first** (`SITE_ROOT/generators/` then the engine). Generators read the product **data** from the site (`build/...`) but their **template** from the engine (`__dirname`). They use the contract `module.exports = { generate(ctx) }` and resolve **site-first by filename**: a generator in `SITE_ROOT/generators/` **shadows** the engine generator of the same name, while a new-named site generator is simply added (so a site can replace `generate-products` or add its own without touching `engine/`). See [docs/generator-migration.md](docs/generator-migration.md) for the contract; the pre-0.2 `generateProductPages(outputDir)` export was **removed in v0.2.1**.
