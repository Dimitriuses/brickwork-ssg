...
- **Known issues / structural inconsistencies** are logged in
  [docs/known-issues.md](docs/known-issues.md) — currently: `_layout` is rendered specially rather
  than through the component pipeline (a `_layout` that behaves like a real component, with `CONTENT`
  as a var, is the fix). Add future bugs there.
