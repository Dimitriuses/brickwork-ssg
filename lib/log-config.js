...
function resolveLogOptions(config, command, args) {
  ...
  // The log output folder is part of the workspace layout (config.json `dirs.log`), not the log
  // block — folders are declared in one place, so this supersedes any log.file.dir. It applies to
  // the file sink when one is enabled; a `--log dir=…` flag below still overrides.
  const logDir = dirNames(undefined, config).log;
  if (opts.file) fileObject(opts).dir = logDir;
  ...
  // A flag may enable the sink (e.g. `--log file=true`) without a dir — fall back to the workspace dir.
  if (opts.file && typeof opts.file === 'object' && !opts.file.dir) opts.file.dir = logDir;
  return opts;
}
