...
// createComponents({ siteRoot, engineRoot, log? }) -> resolution helpers. `log` is optional
// ({ warn }); defaults to silent (e.g. when `ssg add` scans without a build logger).
function createComponents({ siteRoot, engineRoot, log }) {
  const COMPONENTS_DIR = path.join(engineRoot, 'components');
  const SITE_COMPONENTS = path.join(siteRoot, 'components');
