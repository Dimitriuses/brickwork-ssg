...
function createComponents({ siteRoot, engineRoot, engineComponentsDir, log }) {
  ...
  //
  // Memoized per (name, filename): the build resolves the same files repeatedly (once per page × asset
  // kind), and the filesystem doesn't change during a single build, so this caches the up-to-four
  // existsSync probes. The cache lives on the factory (one build), like the registry/subcomponent maps.
  const _resolveCache = new Map();
  function resolveComponentFile(name, filename) {
    const key = name + '\0' + filename;
    ...
    let result = null;
    for (const c of candidates) if (fs.existsSync(c)) { result = c; break; }
    _resolveCache.set(key, result);
    return result;
  }

  // A component's parsed JSON config ({ dependencies, subComponents, head, ... }), site-first. Memoized
  // per name — the config is re-read many times per build (asset collection, sub-component scan, head
  // collection). Callers treat the result as read-only. Do NOT mutate the returned object.
  const _configCache = new Map();
  function readComponentConfig(name) {
    if (_configCache.has(name)) return _configCache.get(name);
    const file = resolveComponentFile(name, `${name}.json`);
    let cfg = { dependencies: [] };
    if (file) {
      try {
