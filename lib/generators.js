...
function readGeneratorRegistry(engineGeneratorsDir, siteGeneratorsDir) {
  ...
  for (const dir of [engineGeneratorsDir, siteGeneratorsDir]) {
    ...
    try {
      ...
      // Route through the logger (honours --quiet / the file sink) instead of a bare console.log.
      log.warn(`Failed to parse generators/registry.json: ${error.message}`, { phase: 'templates' });
    }
  }
  return merged;
}
