...
function runSiteTests(siteRoot) {
  const buildDir = siteDirs(siteRoot).output; // honour a relocated dirs.output (not a hardcoded build/)
  ...
  const testDir = siteDirs(siteRoot).test; // configurable via config.json `dirs`
