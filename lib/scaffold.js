...
`,
  pageJson: (name, layout) => JSON.stringify({
    // `layout` is the grouped { name, vars } form (layout params like header_theme live in vars — the
    // bare top-level header_theme was removed in v0.6.5).
    page: name, title: '', description: '', layout: { name: layout, vars: {} }, components: []
  }, null, 2) + '\n',
...
// Receives the component's vars; return its HTML. helpers = { slugify, escapeHtml, raw, collection, log }.
...
function scaffoldTest(name, ctx) {
  // Site tests are discovered by folder (test/*.test.js) — no registry, like page/builder.
  place(path.join(ctx.dirs.test, `${name}.test.js`), STUBS.test(name), ctx);
}
